...
useEffect(() => {
  ...
  // if (orders.location && Array.isArray(orders.location.coordinates)) {
  //   const coords = orders.location.coordinates;
  ...
  // }
        ...
        console.log("Formatted orders with location:", formatted);
        setOrders(formatted);
        setError(null);
      } catch (err) {
...
const handleMapRedirect = (lat, lng, address) => {
  navigate(`/map?lat=${lat}&lng=${lng}&address=${encodeURIComponent(address)}`);
};
            ...
            <div className={tableClasses.wrapper}>
              <table className={tableClasses.table}>
                <thead className={tableClasses.headerRow}>
                  <tr>
                    {['Order ID', 'Customer', 'Address', 'Items', 'Total Items', 'Price', 'Payment', 'Status','Location'].map(h => (
                    ...
                    return (
                      ...
                      <td className={tableClasses.cellBase}>
                        {/* <div className="text-amber-100/80 text-sm max-w-[200px]">
                         ...
                         <button
                          className="px-3 py-1 text-white bg-amber-500 rounded-md cursor-pointer hover:bg-amber-600 transition-colors"
                         onClick={() => handleMapRedirect(order.location.lat, order.location.lng, `${order.address}, ${order.city}`)}
                           >
                          View Map
                           </button>
                      </td>
