...
const CheckoutPage = () => {
  ...
  const handleSubmit = async e => {
    ...
    const payload = {
     ...
     location: Array.isArray(formData.location) && formData.location.length === 2
  ? [parseFloat(formData.location[0]), parseFloat(formData.location[1])]
  : null
...
console.log("Submitting payload:", payload);
  ...
  {
    enableHighAccuracy: true,     // ✅ Key setting
  ...
  className="mt-3 text-sm text-amber-400 underline cursor-pointer"
>
  Fetch Location
</button>

{formData.location && formData.location.length === 2 && (
  <p className="text-sm text-green-300 mt-2">
    Latitude: {formData.location[0]} | Longitude: {formData.location[1]}
  </p>
)}
